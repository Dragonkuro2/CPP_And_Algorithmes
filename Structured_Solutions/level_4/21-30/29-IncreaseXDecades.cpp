...
stDate IncreaseOneDecade(stDate date) {
	// decade means period of 10 years
	date = IncreaseXYearFaster(date, 10);
	return date;
}

stDate IncreaseXDecade(stDate date, int NumberOfDecadesToAdd) {
	if (NumberOfDecadesToAdd >= 0) {
...
int main() {
	stDate date;
	int NumberOfDecadesToAdd;

	date = ReadDate("Enter The Date: ");
	NumberOfDecadesToAdd = ReadPositiveNumber("Enter the number of decades to add: ");
	date = IncreaseXDecade(date, NumberOfDecadesToAdd);
	PrintDate("Date After Adding " + to_string(NumberOfDecadesToAdd) + " Decade: ", date);

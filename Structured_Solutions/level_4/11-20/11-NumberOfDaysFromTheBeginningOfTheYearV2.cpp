...
string ReverseDate(int NumberOfDays, int year) {
	int month = 1;
	int CurrentMountDays;

	while (true) {
		CurrentMountDays = GetMonthDays(year, month);
		if (CurrentMountDays > NumberOfDays)
			break;
		else
			NumberOfDays -= CurrentMountDays;
		month++;
	}
	string date = to_string(NumberOfDays) + "/" + to_string(month) + "/" + to_string(year);
	return date;
}
...
int main() {
	...
	int NumberOfDays = NumberOfDaysFromTheBeginning(year, month, day);

	cout << "Number of days from the beginning of the year: " << NumberOfDays <<  endl;
	cout << "Date of [" << NumberOfDays << "] is: " << ReverseDate(NumberOfDays, year) << endl;

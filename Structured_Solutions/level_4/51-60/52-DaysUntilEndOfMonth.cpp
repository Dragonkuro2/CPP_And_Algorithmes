...
int main() {
	...
	// check for bussiness day
	cout << "\nIs It Bussiness day: ";
	if (IsItBussinessDay(date))
		cout << "Yes, it's a bussiness day\n";
	else
		cout << "No, it's not a bussiness day.\n";

	cout << "\nDays until end of week: " << DaysUntilTheEndOfWeek(date) << " Day(s)." << endl;

	cout << "\nDays until end of month: " << DaysUntilTheEndOfMonth(date) << " Day(s)." << endl;

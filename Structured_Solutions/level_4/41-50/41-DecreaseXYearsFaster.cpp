...
stDate DecreaseXYearsFaster(stDate date, short years)
{
	date.year -= years;

	// check feb month if it is 28 or 29 and correct it.
...
int main() {
	stDate date;
	int NumberOfYearsToDecrease;

	date = ReadDate("Enter The Date: ");
	NumberOfYearsToDecrease = ReadPositiveNumber("Enter the number of years to decrease: ");
	date = DecreaseXYearsFaster(date, NumberOfYearsToDecrease);
	PrintDate("Date After Decreasing " + to_string(NumberOfYearsToDecrease) + " Year: ", date);

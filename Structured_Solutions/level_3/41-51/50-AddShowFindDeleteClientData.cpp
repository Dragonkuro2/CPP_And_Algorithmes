...
void SaveInfoToFile(stUserInfo client, string filename) {
	...
	// open the file based on the mood we chouse
...
vector <stUserInfo> LoadClientsData(string filename, string delim) {
	...
	if (MyFile.is_open()) {
		...
		while (getline(MyFile, line)) {
			vClients.push_back(ConvertLineToRecord(line, delim)); // convert from vector to struct and then push the data to the vector
		}
		MyFile.close();
	}
	return vClients;
...
void SaveAllVectorsToFile(string filename, vector <stUserInfo>& vClients) {
	fstream MyFile;
	MyFile.open(filename, ios::out | ios::trunc);
	if (MyFile.is_open()) {
		for (stUserInfo line : vClients) {
...
void DeleteClient(string filename) {
	...
	if (ClientAddress != -1) {
		PrintClientDetails(vClients[ClientAddress]);
		choice = ReadNumberInRange("Are you sure you want to delete this client [0]:No [1]:Yes : ", 0, 1);
		if (choice) {
			vClients.erase(vClients.begin() + ClientAddress);
			SaveAllVectorsToFile(filename, vClients);
...
int main() {
	...
	AddUser(FileName);
	cout << "\n\nFind User function:" << endl;
	FindClient(FileName);
	cout << "\n\nPrint All Users function:" << endl;
	PrintAllClientData(FileName);
	cout << "\n\nDelete User function:" << endl;
